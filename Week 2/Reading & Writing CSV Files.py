...
Correct
Awesome! We need to have the exact same amount of variables on the left side of the equals
sign as the length of the sequence on the right side when unpacking rows into individual variable
...
Not quite. We have to explicitly open the file with open() as before we can access the data within.
...
Correct
Correct
Great work! Parsing a file means analyzing its contents to correctly structure the data.
As long as we know what the data is, we can organize it in a way our script can use effectively.

Reading


Incorrect
Not quite. Reading a file implies directly accessing data without properly structuring its contents in a way that can be used.

Incorrect
Not quite. We want to structure the data, not write any new data to the file itself.
"""
